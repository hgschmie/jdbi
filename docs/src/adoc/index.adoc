...
In order to run on JDK 8, some dependencies, such as https://github.com/ben-manes/caffeine[the caffeine caching library^] need to be explicitly version managed to a version that is supported on JDK 8.
...
Since link:{jdbidocs}/core/mapper/ColumnMapper.html[ColumnMapper^] is a functional interface, they can be provided inline to a
query using a lambda expression:
...
| link:{jdbidocs}/core/extension/Extensions.html[Extensions^] | allowProxy
    | boolean  | `true`
...
All methods except `finalize()` can be overridden by the extension type (e.g. through a _interface default method_).
...
Here is an example for a extension handler customizer that logs every method handler invocation. It gets registered as a global extension handler customizer
...
<1> An extension factory that implements only the link:{jdbidocs}/core/extension/ExtensionFactory.html#accepts(java.lang.Class)[ExtensionFactory#accepts()^]
method can be written as a lambda.
...
<2> The extension handler customizer is only applied to a annotated method.
...
The extension framework processes all annotations that use this meta-annotation. It will instantiate the extension configurer by locating a no-arguments
constructor. If this can not be found, an exception will be thrown.
